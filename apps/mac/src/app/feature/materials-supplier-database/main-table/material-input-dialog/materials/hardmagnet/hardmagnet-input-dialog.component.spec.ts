...
import { HardmagnetInputDialogComponent } from './hardmagnet-input-dialog.component';

describe('HardmagnetInputDialogComponent', () => {
  let component: HardmagnetInputDialogComponent;
  let spectator: Spectator<HardmagnetInputDialogComponent>;
  ...
  const createComponent = createComponentFactory({
    component: HardmagnetInputDialogComponent,
    imports: [
      CommonModule,
  ...
  describe('ngAfterInit', () => {
    const strOpt = (id: number, title: string) =>
      ({ id, title } as StringOption);
    it('should set stdDoc with materialName update', () => {
      const so = strOpt(3, 'test');
      const stdDoc = strOpt(3, 'stdDoc for test');
      component['materialNamesControl'].setValue(so);

      expect(component['materialStandardIdControl'].value).toBe(so.id);
      expect(component['standardDocumentsControl'].value).toStrictEqual(stdDoc);
    });
    it('should reset stdDoc on reset of materialName', () => {
      const so = strOpt(undefined, 'test');
      component['materialNamesControl'].setValue(so);
      component['materialNamesControl'].reset();

      expect(component['materialStandardIdControl'].value).toBeFalsy();
      expect(component['standardDocumentsControl'].value).toBeFalsy();
    });
  });
