...
@Component({
  selector: 'mac-estimation-matrix-datagrid',
  imports: [
    // ag grid
    AgGridModule,
    // default
  ...
  templateUrl: './estimation-matrix-datagrid.component.html',
})
export class EstimationMatrixDatagridComponent
  extends BaseDatagridComponent
  implements OnInit, OnDestroy
{
  private readonly rowData$ = this.dataFacade.estimationMatrixResult$;
  private params: IServerSideGetRowsParams<EstimationMatrix>;
  ...
  public ngOnInit(): void {
    ...
    this.rowData$
      ...
      .subscribe((result) => {
        if (result.data) {
          const obj = result as EstimationMatrixResponse;
          const p: EstimationMatrixResult = {
            rowData: obj.data,
  ...
  private createServerSideDataSource(): IServerSideDatasource {
    return {
      getRows: (params: IServerSideGetRowsParams<EstimationMatrix>): void => {
        this.params?.fail();
        this.params = params;
        // fetch new data from request
        this.dataFacade.fetchEstimationMatrix(
