/* eslint-disable max-lines */
import { MaterialClass } from '@mac/msd/constants';
import {
  AluminumMaterial,
  AluminumMaterialResponse,
  CeramicMaterial,
  CeramicMaterialResponse,
  CopperMaterial,
  CopperMaterialResponse,
  LubricantMaterial,
  LubricantMaterialResponse,
  PolymerMaterial,
  PolymerMaterialResponse,
  SteelMaterial,
  SteelMaterialResponse,
} from '@mac/msd/models';

export const msdServiceSteelMockResponse: SteelMaterialResponse[] = [
  {
    id: 127,
    castingMode: undefined,
    castingDiameter: undefined,
    minDimension: 0,
    maxDimension: 0,
    co2PerTon: 2183,
    rating: 'RSI',
    steelMakingProcess: 'BF + BOF',
    releaseDateYear: 2021,
    releaseDateMonth: 10,
    releaseRestrictions: '',
    productCategory: 'strip',
    manufacturerSupplier: {
      id: 442,
      name: 'ArcelorMittal Tubarao',
      plant: 'Tubarao',
      manufacturer: false,
      country: 'BR',
      sapSupplierIds: ['0000000000000', '0000000000001'],
      businessPartnerIds: [20, 31],
    },
    materialStandard: {
      id: 57,
      materialName: 'C80M',
      standardDocument: 'S 130002',
      materialNumber: ['1.1234'],
      wiamId: 'WiamID',
      stoffId: 'StoffID',
    },
    materialClass: 'st',
    selfCertified: false,
    minRecyclingRate: 55,
    maxRecyclingRate: 55,
    blocked: false,
  },
  {
    id: 128,
    castingMode: undefined,
    castingDiameter: undefined,
    minDimension: 0,
    maxDimension: 0,
    co2PerTon: 2183,
    rating: undefined,
    steelMakingProcess: 'BF + BOF',
    releaseDateYear: 2004,
    releaseDateMonth: 6,
    releaseRestrictions: '',
    productCategory: 'strip',
    manufacturerSupplier: {
      id: 442,
      name: 'ArcelorMittal Tubarao',
      plant: 'Tubarao',
      manufacturer: true,
      country: 'BR',
    },
    materialStandard: {
      id: 57,
      materialName: 'C45',
      standardDocument: 'S 130001',
      materialNumber: ['1.1234', '1.2345'],
    },
    materialClass: 'st',
    selfCertified: false,
    minRecyclingRate: 22,
    maxRecyclingRate: 77,
    blocked: false,
  },
];

export const msdServiceSteelMockResult: SteelMaterial[] = [
  {
    id: 127,
    blocked: false,
    manufacturerSupplierId: 442,
    manufacturerSupplierName: 'ArcelorMittal Tubarao',
    manufacturerSupplierPlant: 'Tubarao',
    manufacturerSupplierCountry: 'BR',
    manufacturerSupplierRegion: 'AM',
    materialClass: MaterialClass.STEEL,
    selfCertified: false,
    manufacturerSupplierSapSupplierIds: ['0000000000000', '0000000000001'],
    manufacturerSupplierBusinessPartnerIds: [20, 31],
    materialStandardId: 57,
    materialStandardMaterialName: 'C80M',
    materialStandardStandardDocument: 'S 130002',
    materialStandardStoffId: 'StoffID',
    materialStandardWiamId: 'WiamID',
    productCategory: 'strip',
    productCategoryText:
      'materialsSupplierDatabase.productCategoryValues.strip',
    minDimension: 0,
    maxDimension: 0,
    co2PerTon: 2183,
    castingMode: undefined,
    castingDiameter: undefined,
    rating: 'RSI',
    steelMakingProcess: 'BF + BOF',
    releaseDateYear: 2021,
    releaseDateMonth: 10,
    releaseRestrictions: '',
    materialNumbers: ['1.1234'],
    co2Scope1: undefined,
    co2Scope2: undefined,
    co2Scope3: undefined,
    co2Classification: undefined,
    ratingRemark: undefined,
    ratingChangeComment: undefined,
    referenceDoc: undefined,
    manufacturer: false,
    lastModified: undefined,
    modifiedBy: undefined,
    minRecyclingRate: 55,
    maxRecyclingRate: 55,
  },
  {
    id: 128,
    blocked: false,
    manufacturerSupplierId: 442,
    manufacturerSupplierName: 'ArcelorMittal Tubarao',
    manufacturerSupplierPlant: 'Tubarao',
    manufacturerSupplierCountry: 'BR',
    manufacturerSupplierRegion: 'AM',
    materialClass: MaterialClass.STEEL,
    selfCertified: false,
    manufacturerSupplierSapSupplierIds: undefined,
    manufacturerSupplierBusinessPartnerIds: undefined,
    materialStandardId: 57,
    materialStandardMaterialName: 'C45',
    materialStandardStandardDocument: 'S 130001',
    productCategory: 'strip',
    productCategoryText:
      'materialsSupplierDatabase.productCategoryValues.strip',
    minDimension: 0,
    maxDimension: 0,
    co2PerTon: 2183,
    steelMakingProcess: 'BF + BOF',
    releaseDateYear: 2004,
    releaseDateMonth: 6,
    releaseRestrictions: '',
    castingDiameter: undefined,
    castingMode: undefined,
    co2Scope1: undefined,
    co2Scope2: undefined,
    co2Scope3: undefined,
    co2Classification: undefined,
    rating: undefined,
    ratingRemark: undefined,
    ratingChangeComment: undefined,
    referenceDoc: undefined,
    materialNumbers: ['1.1234', '1.2345'],
    manufacturer: true,
    lastModified: undefined,
    minRecyclingRate: 22,
    maxRecyclingRate: 77,
  },
];

export const msdServicePolymerMockResponse: PolymerMaterialResponse[] = [
  {
    id: 127,
    co2PerTon: 2183,
    releaseDateYear: 2021,
    releaseDateMonth: 10,
    releaseRestrictions: '',
    productCategory: 'strip',
    manufacturerSupplier: {
      id: 442,
      name: 'ArcelorMittal Tubarao',
      plant: 'Tubarao',
      country: 'Brazil',
      sapSupplierIds: ['0000000000000', '0000000000001'],
      businessPartnerIds: [20, 31],
    },
    materialStandard: {
      id: 57,
      materialName: 'C80M',
      standardDocument: 'S 130002',
    },
    materialClass: 'px',
  },
  {
    id: 128,
    co2PerTon: 2183,
    releaseDateYear: 2004,
    releaseDateMonth: 6,
    releaseRestrictions: '',
    productCategory: 'strip',
    manufacturerSupplier: {
      id: 442,
      name: 'ArcelorMittal Tubarao',
      plant: 'Tubarao',
      country: 'Brazil',
    },
    materialStandard: {
      id: 57,
      materialName: 'C45',
      standardDocument: 'S 130001',
      materialNumber: ['sapMaterialNumber'],
    },
    materialClass: 'px',
    generalDescription: 'generalDescription',
  },
];

export const msdServicePolymerMockResult: PolymerMaterial[] = [
  {
    id: 127,
    manufacturerSupplierId: 442,
    manufacturerSupplierName: 'ArcelorMittal Tubarao',
    manufacturerSupplierPlant: 'Tubarao',
    manufacturerSupplierCountry: 'Brazil',
    materialClass: MaterialClass.POLYMER,
    manufacturerSupplierSapSupplierIds: ['0000000000000', '0000000000001'],
    manufacturerSupplierBusinessPartnerIds: [20, 31],
    materialStandardId: 57,
    materialStandardMaterialName: 'C80M',
    materialStandardStandardDocument: 'S 130002',
    productCategory: 'strip',
    productCategoryText:
      'materialsSupplierDatabase.productCategoryValues.strip',
    co2PerTon: 2183,
    releaseDateYear: 2021,
    releaseDateMonth: 10,
    releaseRestrictions: '',
    co2Scope1: undefined,
    co2Scope2: undefined,
    co2Scope3: undefined,
    co2Classification: undefined,
    referenceDoc: undefined,
    generalDescription: undefined,
    materialNumbers: undefined,
  },
  {
    id: 128,
    manufacturerSupplierId: 442,
    manufacturerSupplierName: 'ArcelorMittal Tubarao',
    manufacturerSupplierPlant: 'Tubarao',
    manufacturerSupplierCountry: 'Brazil',
    materialClass: MaterialClass.POLYMER,
    manufacturerSupplierSapSupplierIds: undefined,
    manufacturerSupplierBusinessPartnerIds: undefined,
    materialStandardId: 57,
    materialStandardMaterialName: 'C45',
    materialStandardStandardDocument: 'S 130001',
    productCategory: 'strip',
    productCategoryText:
      'materialsSupplierDatabase.productCategoryValues.strip',
    co2PerTon: 2183,
    releaseDateYear: 2004,
    releaseDateMonth: 6,
    releaseRestrictions: '',
    co2Scope1: undefined,
    co2Scope2: undefined,
    co2Scope3: undefined,
    co2Classification: undefined,
    referenceDoc: undefined,
    lastModified: undefined,
    generalDescription: 'generalDescription',
    materialNumbers: ['sapMaterialNumber'],
  },
];

export const msdServiceAluminumMockResponse: AluminumMaterialResponse[] = [
  {
    id: 127,
    co2PerTon: 2183,
    releaseRestrictions: '',
    manufacturerSupplier: {
      id: 442,
      name: 'ArcelorMittal Tubarao',
      plant: 'Tubarao',
      country: 'Brazil',
    },
    materialStandard: {
      id: 57,
      materialName: 'C80M',
      standardDocument: 'S 130002',
    },
    materialClass: 'al',
    productCategory: 'raw',
  },
  {
    id: 128,
    co2PerTon: 2183,
    releaseRestrictions: '',
    manufacturerSupplier: {
      id: 442,
      name: 'ArcelorMittal Tubarao',
      plant: 'Tubarao',
      country: 'Brazil',
    },
    materialStandard: {
      id: 57,
      materialName: 'C45',
      standardDocument: 'S 130001',
    },
    materialClass: 'al',
    productCategory: 'raw',
  },
];

export const msdServiceAluminumMockResult: AluminumMaterial[] = [
  {
    id: 127,
    manufacturerSupplierId: 442,
    manufacturerSupplierName: 'ArcelorMittal Tubarao',
    manufacturerSupplierPlant: 'Tubarao',
    manufacturerSupplierCountry: 'Brazil',
    materialStandardId: 57,
    materialStandardMaterialName: 'C80M',
    materialStandardStandardDocument: 'S 130002',
    materialClass: MaterialClass.ALUMINUM,
    productCategory: 'raw',
    co2PerTon: 2183,
    releaseRestrictions: '',
    co2Scope1: undefined,
    co2Scope2: undefined,
    co2Scope3: undefined,
    co2Classification: undefined,
    lastModified: undefined,
  },
  {
    id: 128,
    manufacturerSupplierId: 442,
    manufacturerSupplierName: 'ArcelorMittal Tubarao',
    manufacturerSupplierPlant: 'Tubarao',
    manufacturerSupplierCountry: 'Brazil',
    materialStandardId: 57,
    materialStandardMaterialName: 'C45',
    materialStandardStandardDocument: 'S 130001',
    materialClass: MaterialClass.ALUMINUM,
    productCategory: 'raw',
    co2PerTon: 2183,
    releaseRestrictions: '',
    co2Scope1: undefined,
    co2Scope2: undefined,
    co2Scope3: undefined,
    co2Classification: undefined,
    lastModified: undefined,
  },
];

export const msdServiceCopperMockResponse: CopperMaterialResponse[] = [
  {
    id: 127,
    castingMode: undefined,
    castingDiameter: undefined,
    maxDimension: 0,
    co2PerTon: 2183,
    releaseRestrictions: '',
    productCategory: 'strip',
    manufacturerSupplier: {
      id: 442,
      name: 'ArcelorMittal Tubarao',
      plant: 'Tubarao',
      country: 'Brazil',
    },
    materialStandard: {
      id: 57,
      materialName: 'C80M',
      standardDocument: 'S 130002',
      materialNumber: ['2.1234'],
    },
    materialClass: 'cu',
    productionProcess: 'mining',
  },
  {
    id: 128,
    castingMode: undefined,
    castingDiameter: undefined,
    maxDimension: 0,
    co2PerTon: 2183,
    releaseRestrictions: '',
    productCategory: 'strip',
    manufacturerSupplier: {
      id: 442,
      name: 'ArcelorMittal Tubarao',
      plant: 'Tubarao',
      country: 'Brazil',
    },
    materialStandard: {
      id: 57,
      materialName: 'C45',
      standardDocument: 'S 130001',
      materialNumber: ['2.1234', '2.2345'],
    },
    materialClass: 'cu',
    productionProcess: undefined,
  },
];

export const msdServiceCopperMockResult: CopperMaterial[] = [
  {
    id: 127,
    manufacturerSupplierId: 442,
    manufacturerSupplierName: 'ArcelorMittal Tubarao',
    manufacturerSupplierPlant: 'Tubarao',
    manufacturerSupplierCountry: 'Brazil',
    materialClass: MaterialClass.COPPER,
    materialStandardId: 57,
    materialStandardMaterialName: 'C80M',
    materialStandardStandardDocument: 'S 130002',
    productCategory: 'strip',
    productCategoryText:
      'materialsSupplierDatabase.productCategoryValues.strip',
    maxDimension: 0,
    co2PerTon: 2183,
    castingMode: undefined,
    castingDiameter: undefined,
    releaseRestrictions: '',
    materialNumbers: ['2.1234'],
    co2Scope1: undefined,
    co2Scope2: undefined,
    co2Scope3: undefined,
    co2Classification: undefined,
    referenceDoc: undefined,
    lastModified: undefined,
    productionProcess: 'mining',
    productionProcessText:
      'materialsSupplierDatabase.productionProcessValues.cu.mining',
  },
  {
    id: 128,
    manufacturerSupplierId: 442,
    manufacturerSupplierName: 'ArcelorMittal Tubarao',
    manufacturerSupplierPlant: 'Tubarao',
    manufacturerSupplierCountry: 'Brazil',
    materialClass: MaterialClass.COPPER,
    materialStandardId: 57,
    materialStandardMaterialName: 'C45',
    materialStandardStandardDocument: 'S 130001',
    productCategory: 'strip',
    productCategoryText:
      'materialsSupplierDatabase.productCategoryValues.strip',
    maxDimension: 0,
    co2PerTon: 2183,
    releaseRestrictions: '',
    castingDiameter: undefined,
    castingMode: undefined,
    co2Scope1: undefined,
    co2Scope2: undefined,
    co2Scope3: undefined,
    co2Classification: undefined,
    referenceDoc: undefined,
    materialNumbers: ['2.1234', '2.2345'],
    productionProcess: undefined,
    productionProcessText: undefined,
    lastModified: undefined,
  },
];

export const msdServiceCeramicMockResponse: CeramicMaterialResponse[] = [
  {
    id: 127,
    co2PerTon: 2183,
    releaseRestrictions: '',
    productCategory: 'strip',
    manufacturerSupplier: {
      id: 442,
      name: 'ArcelorMittal Tubarao',
      plant: 'Tubarao',
      country: 'Brazil',
    },
    materialStandard: {
      id: 57,
      materialName: 'C80M',
      standardDocument: 'S 130002',
    },
    materialClass: 'ce',
    condition: 'condition',
  },
  {
    id: 128,
    co2PerTon: 2183,
    releaseRestrictions: '',
    productCategory: 'strip',
    manufacturerSupplier: {
      id: 442,
      name: 'ArcelorMittal Tubarao',
      plant: 'Tubarao',
      country: 'Brazil',
    },
    materialStandard: {
      id: 57,
      materialName: 'C45',
      standardDocument: 'S 130001',
    },
    materialClass: 'ce',
    condition: undefined,
  },
];

export const msdServiceCeramicMockResult: CeramicMaterial[] = [
  {
    id: 127,
    manufacturerSupplierId: 442,
    manufacturerSupplierName: 'ArcelorMittal Tubarao',
    manufacturerSupplierPlant: 'Tubarao',
    manufacturerSupplierCountry: 'Brazil',
    materialClass: MaterialClass.CERAMIC,
    materialStandardId: 57,
    materialStandardMaterialName: 'C80M',
    materialStandardStandardDocument: 'S 130002',
    productCategory: 'strip',
    productCategoryText:
      'materialsSupplierDatabase.productCategoryValues.strip',
    co2PerTon: 2183,
    releaseRestrictions: '',
    co2Scope1: undefined,
    co2Scope2: undefined,
    co2Scope3: undefined,
    co2Classification: undefined,
    lastModified: undefined,
  },
  {
    id: 128,
    manufacturerSupplierId: 442,
    manufacturerSupplierName: 'ArcelorMittal Tubarao',
    manufacturerSupplierPlant: 'Tubarao',
    manufacturerSupplierCountry: 'Brazil',
    materialClass: MaterialClass.CERAMIC,
    materialStandardId: 57,
    materialStandardMaterialName: 'C45',
    materialStandardStandardDocument: 'S 130001',
    productCategory: 'strip',
    productCategoryText:
      'materialsSupplierDatabase.productCategoryValues.strip',
    co2PerTon: 2183,
    releaseRestrictions: '',
    co2Scope1: undefined,
    co2Scope2: undefined,
    co2Scope3: undefined,
    co2Classification: undefined,
    lastModified: undefined,
  },
];

export const msdServiceLubricantMockResponse: LubricantMaterialResponse[] = [
  {
    manufacturerSupplier: {
      id: 442,
      name: 'ArcelorMittal Tubarao',
      plant: 'Tubarao',
      country: 'BR',
      sapSupplierIds: ['12345'],
    },
    materialStandard: {
      id: 57,
      materialName: 'C80M',
      standardDocument: 'L 130002',
      materialNumber: ['2.1234'],
      stoffId: '130002-00',
      wiamId: '49075',
    },
    id: 127,
    co2PerTon: 2183,
    materialClass: 'lu',
  },
  {
    manufacturerSupplier: {
      id: 146,
      name: 'ArcelorMittal Tubarao',
      plant: 'Tubarao',
      country: 'BR',
      sapSupplierIds: ['12345'],
    },
    materialStandard: {
      id: 22,
      materialName: 'L122',
      standardDocument: 'L 2',
      stoffId: '130011-00',
      wiamId: '49022',
    },
    id: 127,
    co2PerTon: 2213.3,
    materialClass: 'lu',
  },
];

export const msdServiceLubricantMockResult: LubricantMaterial[] = [
  {
    materialClass: MaterialClass.LUBRICANTS,
    id: 127,
    manufacturerSupplierId: 442,
    manufacturerSupplierName: 'ArcelorMittal Tubarao',
    manufacturerSupplierPlant: 'Tubarao',
    manufacturerSupplierCountry: 'BR',
    manufacturerSupplierRegion: 'AM',
    materialStandardId: 57,
    materialStandardMaterialName: 'C80M',
    materialStandardStandardDocument: 'L 130002',
    materialStandardStoffId: '130002-00',
    materialStandardWiamId: '49075',
    co2PerTon: 2183,
    materialNumbers: ['2.1234'],
    lastModified: undefined,
  },
  {
    materialClass: MaterialClass.LUBRICANTS,
    id: 127,
    manufacturerSupplierId: 146,
    manufacturerSupplierName: 'ArcelorMittal Tubarao',
    manufacturerSupplierPlant: 'Tubarao',
    manufacturerSupplierCountry: 'BR',
    manufacturerSupplierRegion: 'AM',
    materialStandardId: 22,
    materialStandardMaterialName: 'L122',
    materialStandardStandardDocument: 'L 2',
    materialStandardStoffId: '130011-00',
    materialStandardWiamId: '49022',
    co2PerTon: 2213.3,
    materialNumbers: undefined,
    lastModified: undefined,
  },
];
