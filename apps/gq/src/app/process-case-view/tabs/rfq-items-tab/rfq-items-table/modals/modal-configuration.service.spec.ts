  ...
  describe('getMenuItemsByStatus', () => {
    describe('OPEN', () => {
      test('should return menu items for OPEN status', () => {
        ...
        expect(result[0].caption).toBe(
          translate('shared.rfqItemsTable.actionMenuItems.startProcess')
        );
        expect(result[1].caption).toBe(
          translate('shared.rfqItemsTable.actionMenuItems.cancelProcess')
        );
      });
    ...
    describe('CANCELLED', () => {
      test('should return menu items for CANCELLED status and set Reopen disabled to true for allowedToReopen', () => {
        ...
        expect(result).toHaveLength(2);
        expect(result[0].caption).toBe(
          translate('shared.rfqItemsTable.actionMenuItems.showHistory')
        );
        expect(result[1].caption).toBe(
          translate('shared.rfqItemsTable.actionMenuItems.reopenProcess')
        );
